import '../../../Cards/Pagination/Pagination.css';
import { FcPrevious, FcNext } from 'react-icons/fc';
import { pathReview } from '../../../../helpers/path';
import { Link, useParams } from 'react-router-dom';

const PaginationReview = ({ totalPages }) => {
  // params consulta
  ...
  for (let i = 0; i < totalPages; i++) {
    numbers.push(
      <Link to={`${pathReview(i + 1, name, city)}`} key={i}>
      <div
      key={i}
  ...
  return (
    <div className="containerPagination">
      ...
      {(Number(id) > 1) &&
        (
        <Link to={`${pathReview(Number(id) - 1, name, city)}`}>
