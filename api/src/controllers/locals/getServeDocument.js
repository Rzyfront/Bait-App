...
module.exports = async (req, res) => {
  try {
    ...
    if (!auth) throw new Error('Token no proporcionado o inválido');

    const { documentId } = jwt.verify(auth, process.env.SECRET_KEY_2);
    if (!documentId) throw new Error('Token no proporcionado o inválido');

    const document = await Document.findByPk(documentId);
    if (!document) throw new Error('Token no proporcionado o inválido');
