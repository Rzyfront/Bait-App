...
module.exports = async (req, res) => {
  try {
   ...
   // si el usuario no se encuentra lanza un error
    ...
    await user.save();
     // Devuelve una respuesta con el estado 201 (Created) y un objeto JSON con la propiedad success en true
    return res.status(201).json({ success: true });
  } catch (err) {
    // Si hay un error, devuelve una respuesta con el estado 400 (Bad Request) y un objeto JSON con el 
