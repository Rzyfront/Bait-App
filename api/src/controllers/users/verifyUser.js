...
module.exports = async (req, res) => {
  try {
    ...
    if (!token) throw new Error('Token no proporcionado o inválido');

    const { id, verified } = jwt.verify(token, process.env.SECRET_KEY_2);
    if (!id || !verified) throw new Error('Token no proporcionado o inválido');

    const userToVerify = await User.findByPk(id);
    if (!userToVerify) throw new Error('Token no proporcionado o inválido');
    ...
    res.status(400).send(`<h1>No se pudo verificar el usuario debido a: ${error.message} </h1>`);
  }
};
